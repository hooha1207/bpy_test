...
sel_pbs = [i.name for i in bpy.context.selected_pose_bones]

for selb in sel_pbs:
    selb = bpy.data.objects[actob_n].pose.bones[selb]
    if type(selb.parent) != type(None):
        top_parent_bn = selb.parent.name
    else:
        top_parent_bn = None
